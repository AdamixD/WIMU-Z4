...
def pad_and_mask(batch: list[tuple[np.ndarray, np.ndarray]]):
    """Pad and mask for regression (VA mode)"""
...
def pad_and_mask_classification(batch: list[tuple[np.ndarray, np.ndarray]]):
    """Pad and mask for classification (Russell 4Q mode)"""
    maxT = max(x.shape[0] for x, _ in batch)
    Xs, Ys, Ms = [], [], []
    for X, Y in batch:
        T = X.shape[0]
        ...
        if pad > 0:
            X = np.pad(X, ((0, pad), (0, 0)), mode="edge")
            Y = np.pad(Y, ((0, pad),), mode="edge")  # Y is 1D for classification
            m = np.zeros((maxT,), np.float32)
            m[:T] = 1
        else:
            m = np.ones((maxT,), np.float32)
        Xs.append(torch.from_numpy(X).float())
        Ys.append(torch.from_numpy(Y).long())  # Long for class indices
        Ms.append(torch.from_numpy(m).float())
    return torch.stack(Xs, 0), torch.stack(Ys, 0), torch.stack(Ms, 0)
